...
class TagService {
  ...
  getTags() async {
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your tags - CODE TAGS:' +
  ...
  updateOrganizedCount(organizedNow) {
    ...
    if (organizedAtDaybreak == '' ||
        organizedAtDaybreak['midnight'] < ms(midnight))
      store.set('organizedAtDaybreak',
          {'midnight': ms(midnight), 'organized': organizedNow}, 'disk');
  ...
  getCloudAchievements() async {
    var response = await ajax('post', 'query',
        {'tags': [], 'sort': 'newest', 'timeHeader': true, 'from': 1, 'to': 1});

    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your achievements - CODE ACHIEVEMENTS:' +
                response['code'].toString(),
            'yellow');
      return;
    }
  ...
  editHometags(String tag, bool add) async {
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error updating your hometags - CODE HOMETAGS:' +
                response['code'].toString(),
            'yellow');
    }
  ...
  toggleTags(dynamic piv, String type, [selectAll = null]) async {
    ...
    var tagMapPrefix =
        'tagMap' + (type == 'local' ? 'Local' : 'Uploaded') + ':';

    var state =
        store.get('toggleTags' + (type == 'local' ? 'Local' : 'Uploaded'));
    if (state == '') state = {};

    // if false/'', set to true; if true, set to false. This is the toggle.
    if (selectAll != null)
      state[pivId] = selectAll;
    else
      state[pivId] = store.get(tagMapPrefix + pivId) != true;
  ...
  doneTagging(String view) async {
    var tags = store
        .get('currentlyTagging' + (view == 'local' ? 'Local' : 'Uploaded'));

    tags.forEach((tag) => updateLastNTags(tag));

    var state =
        store.get('toggleTags' + (view == 'local' ? 'Local' : 'Uploaded'));
    if (state == '') state = {};
    ...
    state.forEach((id, tagged) {
      ...
      if (cloudId == '' || cloudId == true)
        localPivsToTagUntag[id] = tagged;
      else {
        if (tagged == true)
          cloudPivsToTag.add(cloudId);
        else
          cloudPivsToUntag.add(cloudId);
      }
    });

    if (cloudPivsToTag.length > 0)
      for (var tag in tags) {
        var response = await ajax('post', 'tag',
            {'tag': tag, 'ids': cloudPivsToTag, 'autoOrganize': true});
        if (response['code'] != 200)
          return showSnackbar(
              'There was an error tagging your piv - CODE TAG:' +
                  response['code'].toString(),
              'yellow');
      }

    if (cloudPivsToUntag.length > 0)
      for (var tag in tags) {
        var response = await ajax('post', 'tag', {
          'tag': tag,
          'ids': cloudPivsToUntag,
          'del': true,
          'autoOrganize': true
        });
        if (response['code'] != 200)
    ...
    if ((cloudPivsToTag + cloudPivsToUntag).length > 0)
      queryOrganizedIds(cloudPivsToTag + cloudPivsToUntag);

    var hometags = getList('hometags');
    if (cloudPivsToTag.length > 0 && hometags.isEmpty)
      editHometags(tags[0], true);
    ...
    for (var id in localPivsToTagUntag.keys) {
      ...
      if (pendingTags.length > 0)
        store.set('pendingTags:' + id, pendingTags, 'disk');
      else
        store.remove('pendingTags:' + id, 'disk');
  ...
  getTaggedPivs(dynamic tags, String view) async {
    ...
    var tagMapPrefix =
        'tagMap' + (view == 'local' ? 'Local' : 'Uploaded') + ':';
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your tagged pivs - CODE TAGGED:' +
                response['code'].toString(),
            'yellow');
      return;
    }
    ...
    var queryIds;
    if (view == 'uploaded')
      queryIds = store.get('queryResult')['pivs'].map((v) => v['id']);
    ...
    New.forEach((id) {
      if (!existing.contains(id))
        store.set(tagMapPrefix + id, true);
      else
        existing.remove(id);
    });
    existing.forEach((id) {
  ...
  selectAll(String view, String operation, bool select) {
    ...
    if (view == 'uploaded') {
      ...
      queryResult['pivs'].forEach((piv) {
        if (piv['local'] == true) {
          if (operation == 'delete')
            toggleDeletion(piv['piv'].id, 'uploaded', select);
          if (operation == 'tag') toggleTags(piv['piv'], 'uploaded', select);
        } else {
          if (operation == 'delete')
            toggleDeletion(piv['id'], 'uploaded', select);
  ...
  localQuery(tags, currentMonth, queryResult) {
    ...
    if (yearTag != null && monthTag != null) {
      ...
      if (monthTag == 12)
        ...
        maxDate = DateTime.utc(yearTag, monthTag + 1, 1).millisecondsSinceEpoch;
    }
    ...
    if (currentMonth != '') {
      minDateCurrentMonth = DateTime.utc(currentMonth[0], currentMonth[1], 1)
          .millisecondsSinceEpoch;
      if (currentMonth[1] == 12)
        maxDateCurrentMonth =
            DateTime.utc(currentMonth[0] + 1, 1, 1).millisecondsSinceEpoch;
      else
        maxDateCurrentMonth =
            DateTime.utc(currentMonth[0], currentMonth[1] + 1, 1)
                .millisecondsSinceEpoch;
    }
    ...
    store.getKeys('^pendingTags:').forEach((k) {
      ...
      if (minDate > ms(piv.createDateTime) || maxDate < ms(piv.createDateTime))
        return;
      if (monthTag != null &&
          yearTag == null &&
          piv.createDateTime.toUtc().month != monthTag) return;
      ...
      var yearMonth = piv.createDateTime.toUtc().year.toString() +
          ':' +
          piv.createDateTime.toUtc().month.toString();
      if (queryResult['timeHeader'] != null) {
        if (queryResult['timeHeader'][yearMonth] == null)
          queryResult['timeHeader'][yearMonth] = true;
      }

      if (currentMonth != '') {
        if (minDateCurrentMonth > ms(piv.createDateTime) ||
            maxDateCurrentMonth < ms(piv.createDateTime)) return;
        queryResult['pivs']
            .add({'date': ms(piv.createDateTime), 'piv': piv, 'local': true});
      } else
    ...
    if (currentMonth == '' && localPivsToAdd.length > 0) {
      ...
      localPivsToAdd.forEach((piv) {
        if (ms(piv.createDateTime) > ms(lastDate))
          lastDate = piv.createDateTime;
      });
      ...
      if (month == 12)
        maxDateCurrentMonth =
            DateTime.utc(year + 1, 1, 1).millisecondsSinceEpoch;
      else
        maxDateCurrentMonth =
            DateTime.utc(year, month + 1, 1).millisecondsSinceEpoch;
  ...
  getLocalAchievements(pageIndex) async {
    ...
    store.getKeys('^pendingTags:').forEach((key) {
      ...
      if (piv == null) return;
      if (page['from'] > ms(piv.createDateTime) ||
          page['to'] < ms(piv.createDateTime)) return;
    ...
    localCount.keys.forEach((tag) {
      ...
      if (matchingRow == -1)
        output.add([tag, localCount[tag]]);
      else
        output[matchingRow][1] += localCount[tag];
    });
    ...
    if (organized != '')
      output.add(['All time organized', store.get('organized')['total']]);
  ...
  queryPivs([refresh = false, preserveMonth = false]) async {
    ...
    if ((store.get('queryResult') != '' ||
            store.get('queryInProgress') == true) &&
        refresh == false &&
        listEquals(tags, queryTags)) return;

    var currentMonth = store.get('currentMonth');
    if (preserveMonth == true && currentMonth != '')
      return queryPivsForMonth(currentMonth);
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your pivs - CODE QUERY:A:' +
                response['code'].toString(),
            'yellow');
    ...
    if (queryResult['lastMonth'] == null)
      store.remove('currentMonth');
    else {
      var lastMonth = queryResult['lastMonth'][0].split(':');
      store.set(
          'currentMonth', [int.parse(lastMonth[0]), int.parse(lastMonth[1])]);
    }
    ...
    if (queryResult['total'] > 0 &&
        queryResult['lastMonth'] != null &&
        queryResult['lastMonth'][1] < queryResult['pivs'].length) {
      queryResult['pivs']
          .removeRange(queryResult['lastMonth'][1], queryResult['pivs'].length);
    }
    ...
    if (tags.contains('o::')) {
      ...
      queryOrganizedIds(queryResult['pivs']
          .where((v) => v['local'] == null)
          .map((v) => v['id'])
          .toList());

    if (queryResult['total'] > 0 &&
        queryResult['lastMonth'] != null &&
        queryResult['pivs'].length < queryResult['lastMonth'][1]) {
      queryResult['pivs'] = [
        ...queryResult['pivs'],
        ...List.generate(
            queryResult['lastMonth'][1] - queryResult['pivs'].length,
    ...
    if (queryResult['total'] == 0 ||
        queryResult['pivs'].last['placeholder'] == null) return 200;
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your pivs - CODE QUERY:B:' +
                response['code'].toString(),
            'yellow');
      return response['code'];
    }
    ...
    secondQueryResult =
        localQuery(tags, store.get('currentMonth'), secondQueryResult);
    ...
    if (tags.contains('o::')) {
      ...
      queryOrganizedIds(secondQueryResult['pivs']
          .where((v) => v['local'] == null)
          .map((v) => v['id'])
          .toList());
  ...
  queryPivsForMonth(dynamic currentMonth) async {
    ...
    var currentMonthTags = [
      'd::' + currentMonth[0].toString(),
      'd::M' + currentMonth[1].toString()
    ];
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your pivs - CODE QUERY:C:' +
                response['code'].toString(),
            'yellow');
      store.remove('queryInProgress');
      return response['code'];
    ...
    // We don't await on purpose
      ...
      queryOrganizedIds(queryResultForPivs['pivs']
          .where((v) => v['local'] == null)
          .map((v) => v['id'])
          .toList());
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your pivs - CODE QUERY:D:' +
                response['code'].toString(),
            'yellow');
      store.remove('queryInProgress');
      return response['code'];
  ...
  deleteUploadedPivs(dynamic ids) async {
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error deleting your pivs - CODE DELETE:' +
                response['code'].toString(),
            'yellow');
      return;
    }
  ...
  renameTag(String from, String to) async {
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error renaming the tag - CODE RENAME:' +
                response['code'].toString(),
            'yellow');
      return;
    }
  ...
  deleteTag(String tag) async {
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error deleting the tag - CODE RENAME:' +
                response['code'].toString(),
            'yellow');
      return;
    }
  ...
  toggleDeletion(String id, String view, [selectAll = null]) {
    var key =
        'currentlyDeletingPivs' + (view == 'local' ? 'Local' : 'Uploaded');
    ...
    if (!currentlyDeletingPivs.contains(id))
      currentlyDeletingPivs.add(id);
    else
      currentlyDeletingPivs.remove(id);
  ...
  getMonthEdges() {
    ...
    if (currentMonthIndex == null)
      return {'previousMonth': '', 'nextMonth': ''};
    var previousMonth =
        currentMonthIndex == 0 ? '' : nonWhiteMonths[currentMonthIndex - 1];
    var nextMonth =
        currentMonthIndex == index ? '' : nonWhiteMonths[currentMonthIndex + 1];
  ...
  queryOrganizedIds(dynamic ids) async {
    ...
    if (response['code'] != 200) {
      if (![0, 403].contains(response['code']))
        showSnackbar(
            'There was an error getting your organized pivs - CODE ORGANIZE:' +
                response['code'].toString(),
            'yellow');
      return;
    }
  ...
  computeTimeHeader() {
    ...
    timeHeader.keys.forEach((v) {
      ...
      if (dates[0] < min[0] || (dates[0] == min[0] && dates[1] < min[1]))
        min = dates;
      if (dates[0] > max[0] || (dates[0] == max[0] && dates[1] < max[1]))
        max = dates;
    });
    ...
    for (var year = min[0]; year <= max[0]; year++) {
      for (var month = 1; month <= 12; month++) {
        ...
        var isCurrentMonth =
            year == currentMonth[0] && month == currentMonth[1];
        if (timeHeader[dateKey] == null)
          output.add([year, month, 'white', false]);
        else if (timeHeader[dateKey] == false)
          output.add([year, month, 'gray', isCurrentMonth]);
        else
          output.add([year, month, 'green', isCurrentMonth]);
      }
    }
    var semesters = [[]];
    output.forEach((month) {
      var lastSemester = semesters[semesters.length - 1];
      if (lastSemester.length < 6)
        lastSemester.add(month);
      else
        semesters.add([month]);
    });
    ...
    store.set(
        'yearUploaded',
        semesters[semesters.length - 1 - (currentPage == '' ? 0 : currentPage)
            as dynamic][0][0]);
  }
}
