  ...
  void dispose() {
    ...
    for (int i = 0; i < 100; i++) {
      RenderBox itemRenderBox =
          _getItemRenderBox(i); // Get the RenderBox of the current item
      Offset itemTopLeft = itemRenderBox.localToGlobal(Offset.zero);
      Offset itemBottomRight =
          itemRenderBox.localToGlobal(itemRenderBox.size.bottomRight(Offset.zero));
      if (itemTopLeft.dy < bottomRight.dy && itemBottomRight.dy > topLeft.dy) {
        // The item is visible
        print('Item $i is visible');
      } else {
        // The item is not visible
        print('Item $i is not visible');
      }
    }
  }

  RenderBox _getItemRenderBox(int index) {
    // Get the RenderBox of the item at the specified index
      ...
      return (itemKey.currentContext?.findRenderObject() as RenderBox) ??
          _scrollController.position.viewportDimension > 0
              ? _scrollController.position
                  .createRelativeRect(
                      renderBox: _scrollController.position.viewportDimension,
                      minOffset: index * 100.0,
                      maxOffset: (index + 1) * 100.0,
                      rect: Rect.zero)
                  .toRect()
              : null;
    } else {
